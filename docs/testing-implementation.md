...
Comprehensive test suite added for PsiTransfer S3 storage implementation with MinIO integration for CI/CD and local testing.
...
- Full application startup with S3/MinIO storage
...
2. **Integration Tests** - With MinIO container
...
Automatically manages MinIO Docker containers for integration tests:
- Downloads MinIO image on first run
