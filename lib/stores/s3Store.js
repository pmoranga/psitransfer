  ...
  async create(fid, opts = {}) {
    ...
    try {
      ...
      const multipartCommand = new CreateMultipartUploadCommand({
        ...
        Key: key,
        Metadata: {
          'original-filename': opts.metadata?.name || '',
    ...
    try {
      ...
      for await (const chunk of readStream) {
        ...
        const isLastChunk = metadata.uploadLength &&
                           (metadata.offset + bytesReceived >= metadata.uploadLength);

        while (buffer.length >= this.s3PartSize || (isLastChunk && buffer.length > 0)) {
          const isLastPart = isLastChunk && buffer.length <= this.s3PartSize;
          const partSize = isLastPart ? buffer.length : this.s3PartSize;
          const partData = buffer.slice(0, partSize);
          buffer = buffer.slice(partSize);

          debug(`Uploading part ${metadata.partNumber}, size: ${partSize}`);
