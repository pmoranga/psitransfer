...
│   ├── s3Store.test.js     # S3Store tests with MinIO
...
Integration tests use [Testcontainers](https://node.testcontainers.org/) to automatically start a MinIO container and test:
...
**Note:** First run will download MinIO Docker image (~50MB).
...
Tests S3 storage with MinIO (runs in Docker container):
...
- ✅ S3 storage backend with MinIO connectivity
...
docker run --rm \
  ...
  --entrypoint sh \
  minio/mc:latest \
  -c "mc alias set minio http://localhost:9000 minioadmin minioadmin && mc mb minio/psitransfer-test"
```
...
module.exports = {
  storage: {
    ...
    // MinIO endpoint (not needed for real AWS S3)
...
2. **Integration Tests** - With MinIO in Docker
