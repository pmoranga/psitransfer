...
describe('S3Store Integration Tests (MinIO)', function() {
  ...
  before(async () => {
    ...
    // Override S3 client to use MinIO endpoint
  ...
  describe('append() and multipart upload', () => {
    ...
    it('should handle resumable uploads with multiple appends', async () => {
      ...
      const chunk1 = Buffer.alloc(3 * 1024 * 1024, 'a'); // 3MB
      const chunk2 = Buffer.alloc(3 * 1024 * 1024, 'b'); // 3MB
      ...
      expect(result.offset).to.equal(chunk1.length);

      // Second append
      readStream = Readable.from([chunk2]);
      result = await store.append(fid, readStream, chunk1.length);
